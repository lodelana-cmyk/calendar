...
function weekdayWeeks(start: Date, end: Date): (string | null)[][] {
...
function buildHtml(items: ContentItemWithCampaign[], start: Date, end: Date, label: string): string {
  ...
  const weeksHtml = weekdayWeeks(start, end).map(week => {
...
td{vertical-align:top;width:20%;border:1px solid #e2e8f0;padding:6px;min-height:100px}
...
export function ExportCalendarDialog({
  ...
  const [cEnd,   setCEnd]   = useState(() => { const e = startOfWeek(today); e.setDate(e.getDate()+4); return toDS(e) })
  ...
  const range = useMemo(() => {
    ...
    if (rangeMode === "week") {
      const s = startOfWeek(today); const e = new Date(s); e.setDate(e.getDate()+4)
